...
\begin{abstract}
	\todo[inline]{Reviewer~2: ``The lower bounds result in the Abstract is stated without reference to the faithful semigroup. It is worth making that explicit for clarity. \\ The order of the results listed in the Abstract is different from the order in the body of the paper. Specifically, the paper presents the results (1), (2) and (3) in order but discusses (2), (3) and (1) in that order in the Abstract. This is nitpicky but keeping them consistent will help.''~--- We added faithful non-commutative semigroups in the lower bound statement. We changed the order of the results in the abstract, now upper bounds are stated first. We still would like to keep more general upper bound to be the first one in the abstract and the second one discussed in the paper. We believe that on this level the optimal order of results might be different depending on the space constraints.}
	...
	For the case when the semigroup is commutative, we give a~constructive proof of an~$O(z)$ upper bound. This implies that in commutative settings, complements of sparse matrices can be processed as efficiently as sparse matrices (though
	...
	graph theory.    
    On the other hand, we prove that in general this is not possible: for faithful non-commutative semigroups there
...
\end{mypic}
%
\todo[inline]{changed `semiring' to `semigroup' once in the paragraph below.}
...
in every row such that for any $(S, \circ)$ faithful non-commutative semigroup the minimum number of semigroup
operations
