...
\begin{abstract}
	\vtodo{Reviewer~2: ``The lower bounds result in the Abstract is stated without reference to the faithful semigroup. It is worth making that explicit for clarity. \\ The order of the results listed in the Abstract is different from the order in the body of the paper. Specifically, the paper presents the results (1), (2) and (3) in order but discusses (2), (3) and (1) in that order in the Abstract. This is nitpicky but keeping them consistent will help.''}
...
\end{mypic}
%
What is~the minimum number of~semigroup operations needed 
to~compute these six sums? The answer depends on~the properties of~the underlying semigroup $(S, \circ)$. 
For example, if $S=(\{0,1\}, \oplus)$, then one way is~to compute the sum of~all input variables first and then to~cancel every variable. However this strategy 
does not work for $S=(\{0,1\}, \lor)$. For this semiring,
one can start by~computing all prefix and suffix
sums and then, for every output, to~combine the corresponding prefix and suffix.
...
In~this paper, 
we are interested in lower and
...
\vtodo{Reviewer~1: ``At first, I had the confusion that the same algorithm / circuit has to work for any ``faithful non-commutative'' semigroup, as in the commutative section, and the lower bound rules out small circuits of this kind. It would be good to emphasize that the lower bound works even if the circuit can be constructed based on~$S$.}
...
\vtodo{Reviewer~2: ``Theorem 1.3 can be split into two parts — one stating the lower bound and the other stating the upper bounds. The proof of the upper bound is short and straightforward from [21], and the main result is the lower bound.''}
...
\atodo{Reviewer~2: ``The paragraphs on Range Queries and Graph Algorithms in Section 1.2 need references to literature. Additionally and more importantly, Range Queries should discuss data structures in the Group model because their lower bound proof relies on [6], which proves a lower bound for a problem in the Group model for data structures.''}

\atodo{Reviewer~2: ``To reiterate the above comment, I think the authors should expand on “”The algorithms (Theorem 1.1 and Corollary 1.2) and hardness results (Theorem 1.3) for the linear operator problem presented in this paper are indeed inspired by some of the known results for the range queries problem.” to discuss [6] and other related work.''}

\atodo{Reviewer~2: ``It will be nice to discuss Algorithms by Yao and by Alon and Schieber from A.3 in the main body of the paper as their upper bound results rely on them.''}
...
problem. The connection to~range queries is~straightforward:
zeros in a~row split this row into a~collection of~ranges
in a~natural way.
...
\item[Graph algorithms.] Various graph path and reachability
...
in an undirected graph can be found by computing the third power (over the integers) of its
adjacency matrix.
...
two parts, $L$~and~$R$. Thanks to~commutativity,
we~can handle the submatrices spanned by~$L$ and~$R$
separately and then merge the results.

Let us split the rows into two parts: the top
...
columns out of it and make a recursive call. 
\todo[inline]{Reviewer~1: ``“note that this requires the commutativity”, this is true, but doesn’t the top part already require commutativity, because L and R may not necessarily be sets of consecutive integers?''~--- Right! Fixed.}
The corresponding recurrence relation is $T(n) \le cn + T(n/2)$
...
\vtodo{Reviewer~3: ``In section 4 after definition 4.1 when the term "initial mark" is defined, my comments are that 1) the definition of "initial mark" and "initial" should be made its own definition (e.g. definition 4.2) and 2) the definition of "initial mark" and "initial" would benefit greatly from one or two simple examples, which shouldn't take much space or time to present.''}
...
\vtodo{Reviewer~1: ``“This allows to capture in this definition important cases of idempotent semigroups.” I’m not sure if I understand this sentence. This definition does not imply idempotence, nor is implied by idempotence.''}
...
\atodo{Please add a nice acknowledgment to the reviewers.}
