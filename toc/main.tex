...
\usepackage{amssymb}
\usepackage[colorinlistoftodos, color=green!20, disable]{todonotes}
...
The upper bound in this theorem follows straightforwardly from the results in~\cite{DBLP:conf/stoc/Yao82} and the lower bound is the main result for non-commutative case.
...
\todo[inline]{Reviewer~2: ``Theorem 1.3 can be split into two parts — one stating the lower bound and the other stating the upper bounds. The proof of the upper bound is short and straightforward from [21], and the main result is the lower bound.'~--- We prefer to keep it as one statement: we would like to make more visible the fact that we have tight lower and upper bounds. We rephrased the theorem to move the main result forward and we added a comment after the theorem mentioning that the upper bound is easy to prove.}.
...
\todo[inline]{Reviewer~1: ``The statement is too casual, consider rephrasing it''~--- We rephrased the lemma.}
...
\todo[inline]{Reviewer~2: ``The result from [6] used in the proof of Theorem 1.3 must be explicitly stated. ''~--- Done.}
\begin{proof}[Proof of Theorem~\ref{thm:lowerbound}]
	\todo[inline]{Reviewer~1: ``Technically, Theorem 1.3 is stated for general strongly non-commutative semigroups, while Theorem 4.6 is stated for idempotent X. Consider integrating the argument in section 4.3 to the proof, or moving it to an earlier place.''~--- We formulated the reduction to idempotent case as a separate lemma and updated the proof of the theorem accordingly.}
