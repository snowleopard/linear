...
\expref{Theorem}{thm:upperbound} generalizes easily to show that $Ax$ for a~matrix
...
The following corollary generalizes \expref{Theorem}{thm:upperbound}
...
a~contiguous range. The algorithms (\expref{Theorem}{thm:upperbound}) and hardness
results (\expref{Theorem}{thm:lowerbound}) for the linear operator problem presented
...
the range queries problem in \expref{Sections}{subseq:rmqapp} \expref{and}{subsec:approaches}.
...
\expref{Theorem}{thm:upperbound} and \expref{Corollary}{cor:matrixmultiplication} give
...
\expref{Theorem}{thm:upperbound} and \expref{Corollary}{cor:matrixmultiplication}
...
As we discuss in \expref{Section}{sec-dense-graph}, \expref{Theorem}{thm:upperbound}
...
\expref{Theorem}{thm:upperbound} states that linear operators on complements of
...
In the \emph{range queries problem}, one is given
...
the range queries problem, each word has a~form $x_l\circ x_{l+1}\circ \dotsb \circ x_r$).
We use
...
This section is devoted to the proofs of \expref{Theorem}{thm:upperbound} and
\expref{Corollary}{cor:matrixmultiplication}, which we % remind %% ToC edit1 -->
...
with \expref{Theorem}{thm:lowerbound} that states that in the
...
be~used as~building blocks in the proof of \expref{Theorem}{thm:upperbound}.
...
linear-size circuits for dense linear operators. % Namely, %% ToC edit1 -->
Specifically,  %% <--
we prove \expref{Theorem}{thm:lowerbound}
...
For the commutative case it is known that the range queries problem is non-linear
(Chazelle--Rosenberg~\cite{DBLP:journals/ijcga/ChazelleR91}).  %% <--
...
By \expref{Theorem}{thm:equivalence} if non-commutative dense linear
