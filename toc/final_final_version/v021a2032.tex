...
%% version kuli43.tex
%% last updated LB 2025-11-07
%% version kuli41.tex
%% last updated AK 2025-11-06
%% version kuli40.tex
...
the %% AU edit4 (AK) a -> the
complement $A \in \{0,1\}^{6 \times 6}$ of~the identity matrix. In~this case, the $i$-th output~$y_i$ (for $i=1, \dotsc, 6$) is~equal to the sum of~all input variables $x_1, \dotsc, x_6$ except for $x_i$.
%
\begin{mypic}
	\begin{scope}[scale=.5]
		\foreach \i in {1,...,6}
...
Our first main result shows that in the commutative case,
the %% AU edit4 (AK)
complements
...
%% ToC edit5 while APSP makes sense for digraphs, the notion of "complement"
%% would require clarification, and I think the gain is not worth it.
%% It seems like limiting this subsubsection to undirected graphs
%% would be reasonable.  -->
In this paper, by ``graphs'' we mean ``simple graphs,'' \ie,
undirected graphs without loops and parallel edges.
...
and (ii) the number of triangles in % an undirected %% ToC edit1 -->
a %% <--
graph can be found by
...
whereas an algorithm with the same efficiency for
the %% AU edit4 (AK)
complements of sparse graphs
%% AU edit4 (AK) comment out the next two lines
% ($O(n)$ % anti-edges) %% ToC edit1 -->
...
time $O(n^2)$, but achieving the same time complexity for
the %% AU edit4 (AK)
complements of sparse
graphs is more complicated.
\expref{Theorem}{thm:upperboundold} and \expref{Corollary}{cor:matrixmultiplicationold} give
a~black-box way to solve these two problems on
the %% AU edit4 (AK)
complements of sparse graphs in
time $O(n^2)$.
...
%% ToC edit5 you need to clarify whether self-loops are permitted in digraphs,
%% and what "complement" means for digraphs -->
The \emph{diagonal} $\Delta(V)$ of the set $V$ is the set $\{(x,x)\mid x\in V\}$.
By \emph{digraphs} (directed graphs) we mean pairs $G=(V,E)$ of sets where
$E\subseteq V\times V$.  So, self-loops, \ie, edges in $\Delta(V)$, are
permitted.  If we wish to exclude such edges, we speak of \emph{loop-free}
digraphs.  The \emph{complement} of $G$ is $(V, V\times V\setminus E)$.
The \emph{loop-free complement} of a loop-free digraph $G$ is
$(V, V\times V \setminus \Delta(V)\setminus E)$.  Graphs can be viewed
as loop-free digraphs where $E$ is a symmetric relation on $V$.  By the
\emph{complement of a graph} we mean its loop-free complement.

One of the algebraic data structures developed in the functional programming
community for the representation and manipulation of digraphs
$G=(V,E)$ is based on the following operations:
\begin{itemize}
...
\item join: $G_1 \ast G_2 = (V_1 \cup V_2, E_1 \cup E_2 \cup (V_1 \times V_2))$, including
  the resulting self-loops, if any.
\end{itemize}
...
(transitive tournament) which has density $1/2$.  %% ToC edit5 note: by definition,
%% tournaments do not have self-loops.  I am not suggesting to make this comment.

It was not known whether the
complements of sparse graphs admit such a concise representation.
%% ToC edit3 is it true that even the existence of such a representation
%% was not known?  ATTN3
%%
%% AM: Yes, that is true. In fact, this specific question motivated our research.
%% ToC edit5 This is interesting! Readers should know about this! -->
In fact, this specific question motivated our research.  %% <-- this line added ATTN5
Our result gives a constructive positive answer:
\expref{Theorem}{thm:upperboundold} yields an
efficient algorithm for deriving a~linear-size algebraic graph representation
for the complements of sparse digraphs.
%
%% ToC edit5 it seems nice to add this comment from your cover letter -->
It easily follows that the same result is true for graphs and for
loop-free digraphs.  %% <--
...
\expref{Theorem}{thm:upperboundold} states that linear operators on
the %% AU edit4 (AK)
complements of
...
\begin{theorem}[\expref{Theorem}{thm:lowerboundold} restated]
...
for the \emph{range minimum query}
problem %% AU edit4 (AK)
(\ie, for a~semigroup
...
\begin{tocinfo}[kulikov]
    ...
    %% AU edit4 (AK) change the e-mail address
    alexander\tocdot{}s\tocdot{}kulikov\tocat{}gmail\tocdot{}com\\
...
\begin{tocinfo}[mikhailin]
    ...
    %% AU edit4 (AK) add a link to DBLP (Ivan doesn't have a webpage)
    \url{https://dblp.org/pid/40/11440.html}
\end{tocinfo}
...
\begin{tocinfo}[podolskii]
    ...
    %\url{https://math.tufts.edu/people/faculty/vladimir-podolskii}
	\url{https://engineering.tufts.edu/cs/people/faculty/vladimir-podolskii}
	%% AU edit6 (VP) I updated the link to the same one, but in CS department. There is no better link, unfortunately
    %% ToC edit5 <-- added this URL; if there is a better URL, please replace
    %% I think https://facultyprofiles.tufts.edu/vladimir-podolskii is worse
\end{tocinfo}
\end{tocauthors}
...
His scientific interests include algorithms, circuit complexity, and Computer Science education.
He~coauthored %% ToC edit5 <-- (co-authored --> coauthored)
three books and sixteen massive open online courses on~algorithms and discrete mathematics with over a~million enrolled students.
In~his spare time, he~enjoys discussing
the %% ToC edit5 <-- inserted
...
%%   your education and degrees
  ...
  Ivan did his \phd{} studies at~the University of~California San Diego
  under the supervision of~Russell Impagliazzo.  %% ToC edit5 <-- years of study or year
  %% of graduation?  ATTN5
\end{tocabout}
\begin{tocabout}[mokhov]
  \textsc{Andrey Mokhov} is a software engineer at~Jane Street Singapore, and
  ...
  studies (2005--2009), Andrey worked on asynchronous circuits and concurrency theory under the supervision of Alex Yakovlev.
  In~2014, he~became interested in functional programming and software build systems, which
  eventually led him to writing more and more code, and in~2019, he switched from
  academia to~industry, joining the Jane Street's Tools and Compilers team.
...
Mathematical Institute.  %% ToC edit5 <-- year?
 %% AU edit6 (VP) I am not sure what the suggestion is here, it seems that years for both thesis defenses are provided
His research areas are circuit complexity, its applications to~databases augmented with
ontologies, min-plus geometry. He is an Associate Professor at Tufts.
\end{tocabout}
\end{tocaboutauthors}
