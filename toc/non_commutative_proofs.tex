...
\vtodo{Reviewer~1: ``At first, I wasn’t sure what this paragraph is trying to say. Later, I realized that this paragraph is trying to justify that non-increasing or non-range circuits can make sense. Make it more explicit.'' (Sasha: perhaps, he means a different place. Please check the review.)}
In particular, the idempotency is tricky. For example, it can be used to simulate commutativity: one can turn $xy$ into $yx$, by first multiplying $xy$ by~$y$ from the left and then multiplying the result by $x$ from the right (obtaining $(y(xy))x=(yx)(yx)=yx$). Using similar ideas, one can place new variables inside of already computed products. To get $xyz$ from $xz$, one multiplies it by $xyz$ first from the left and then from the right: $(xyz)xz(xyz)=xy(zxzx)yz=xy(zx)yz=xyz$.
...
\vtodo{Reviewer~1: ``Claim 4.12, 4.13, 4.14: avoid nested proofs.''}
