...
<<<<<<< HEAD
and $t_1$~ones and 
$x \in S^{n \times 1}$\todo{V: let's write just $S^{n}$ throughout the paper?} be an~$n$-dimensional vector of 
formal variables
over a~semigroup $(S, \circ)$. 
How many semigroup operations are required to 
compute $Ax$? This problem generalizes the well-known 
range queries problem and has a~rich variety of applications in
such areas as graph algorithms, 
functional programming languages, circuit complexity, and others. It 
is immediate that $O(t_1+n+m)$\todo{V: isn't the exact complexity $t_1 - m = O(t_1)$?} semigroup operations are 
sufficient. The main question studied in this paper is: 
can $Ax$~be computed using $O(t_0+n+m)$ semigroup 
operations? \todo{V: let's state positive result first and say that it covers Boolean and min-plus}We prove that in general this is not possible: there 
...
<<<<<<< HEAD
and $t_1$ ones and 
$x=(x_1, \dotsc, x_n) \in S^n$~be an~$n$-dimensional vector 
of formal variables 
over a~semigroup~$(S, \circ)$. 
How many semigroup operations are required to 
compute the \emph{linear operator}~$Ax$?
...
to compute $AX$ where $X \in S^{n \times k}$ is a~matrix of formal variables?
In this paper, 
we are interested in lower and upper bounds 
involving~$t_0$ and~$t_1$. 
Matrices with $t_1=O(n)$ are usually called \emph{sparse} 
whereas matrices with $t_0=O(n)$ 
are called \emph{complements of sparse matrices}. 
It is not difficult to see that computing all $m$~outputs
=======
and $t_1$ ones and $x=(x_1, \dotsc, x_n) \in S^n$~be an~$n$-dimensional vector
of formal variables over a~semigroup~$(S, \circ)$. In this paper we study the
complexity of the \emph{linear operator}~$Ax$, i.e. how many semigroup
...
% More generally, we study how many semigroup operations are needed
% to compute $AX$ where $X \in S^{n \times k}$ is a~matrix of formal variables.
where the summation is over the semigroup\footnote{Note that the result of
...
operation~$\circ$. More specifically, we are interested in lower and
upper bounds involving~$t_0$ and~$t_1$.
Matrices with $t_1=O(n)$ are usually called \emph{sparse},
whereas matrices with $t_0=O(n)$
are called \emph{complements of sparse matrices}.
It is not difficult to see that computing all $n$~outputs
>>>>>>> origin/master
of~$Ax$ independently takes
$O(t_1+n+m)$ semigroup operations.
The main question studied in this paper is:
can $Ax$~be computed using $O(t_0+n+m)$ semigroup
operations? Note that this complexity is easy to achieve if $\circ$ has an
inverse: in this case $A$~can be obtained by subtracting a~dense matrix from
all-ones matrix. Our first result states that this is also possible for
...
variables, and $A \in \{0,1\}^{m \times n}$ be a~matrix 
with~$t_0$
zeros. Then, $Ax$ can be computed using at most
$O(t_0+n+m)$ semigroup operations.
\end{restatable}
...
and $X \in S^{n \times n}$ be a~matrix of formal variables. Then,
$AX$ can be computed using at most
$O(n^2)$ semigroup operations.
\end{restatable}

\todo[inline]{AM: If I understand correctly, the general case when
$A \in \{0,1\}^{m \times n}$ and $X \in S^{n \times k}$ has
complexity $O((t_0+m+n)k)$, is that right? Shall we report it here as well?}
...
$A \in \{0,1\}^{n \times n}$ with $t_0=O(n)$ zeros is
$\Theta(n\alpha(n))$ where $\alpha(n)$ is the inverse Ackermann function.

\todo[inline]{AM: Can we have a lower bound for the general case
$A \in \{0,1\}^{m \times n}$? Is it something like $O((m+n)\alpha(t_0))$?}
...
\todo[inline]{AM: I wonder if we can have an interesting corollary about matrix
multiplication in the style of Corollary~\ref{cor:matrixmultiplication}. Can we
say that computing $A X$ in strongly non-commutative case requires
$\Omega(n^2\alpha(n))$ operations? Or is it too good to be true?}
...
one is given a~vector~$x=(x_1, \dotsc, x_n)$ over 
...
matrix~$A$ defines a~subset of the elements of~$x$
...
are indeed inspired 
by some of the known deep results for the range queries problem.\todo{V: shouldn't we keep the intuition that dense matrices is a special case of range query?}
...
as well as a~rich variety of algebraic techniques for the problem in
Section~\ref{subsec:approaches}.
...
that need to be summed up and this subset is not required to be
a~contiguous interval. The algorithms (Theorem~\ref{thm:upperbound} and
...
graphs is (relatively) difficult.
...
we still do not know an $n^{3-\varepsilon}$ (a~constant~$\varepsilon>0$) upper
...
linear operator $A\mathbf{x}$ where the matrix $A$~is given below and $\mathbf{x}=(x_1, \dotsc, x_5)^T$.
...
This section is devoted to the proof of Theorem~\ref{thm:upperbound}.

\upperthm*

Before proving it, we prove two simpler statements to show
how commutativity is important.

\begin{lemma}\label{lemma:easy}
Let $S$~be a semigroup (not necessarily commutative) and let
$A \in \{0,1\}^{m \times n}$ contain at most one zero in every row. Then
$C(A) = O(m+n)$.
\end{lemma}
\begin{proof}
...
at most $\log n$ is $O(\frac{m}{n}\cdot \log^4 n)$.
\end{lemma}
...
$O(t_1+m)$ ranges. We then take a~circuit guaranteed by 
...
it takes $O(t_0)$ additional binary gates to compute all the outputs.

We
compute the matrices $R_0 \times C$ and $R_1 \times C$
separately. The main idea is that $R_0 \times C$ is easy to compute
because it has a~small number of rows (at most $t_0/\log n$), while $R_1 \times C$
...
\[O(t\log t+t_0)=O\left(\frac{t_0}{\log n} \cdot \log t_0+t_0\right)=O(t_0+n)\, ,\]
since $t_0 =O(n\log n)$.
...
ranges of length at most $\log n$ is $O(\frac{m}{n}\cdot\log^4n)$. Hence, all
such short ranges can be computed by a~circuit of
size~$O(m)$. It remains to compute all the ranges of
...
\todo[inline]{Eschyo mogno vsyo perevesti s kvadratnikh matrits na pryamougol'nie.}
