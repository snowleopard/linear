...
\begin{lemma}\label{lem:com_case_one_zero}
...
\begin{lemma}\label{lem:com_case_k_zero}
Let S be a commutative semigroup and let $A \in \{0,1\}^{n \times m}$ contain at most $k$ zeros at every row. Then $\exists \alpha_k: ~ C(Ax) \leq \alpha_k (n+m)$.
\end{lemma}

\begin{proof}
We will prove this by induction on $k$. The base case is provided by Lemma \ref{lem:com_case_one_zero}.

Suppose we already know that Lemma \ref{lem:com_case_k_zero} is true for all $k' \leq k$. We will prove that it is true for the case $k$. In order to do this we will partition our matrix into $3$ submatrix. We will randomly color each column into red or blue color. Let $R$ be the set of red columns and $B$ be the set of blue. Let $X$ be set of all rows with exactly $k-1$ zeroes and $Y$ be the rest. We will consider following submatrices separatelly: $R \times X$, $B \times X$, $R \sqcup B \times Y$.
...
\draw (0,0) -- (2.5,0) -- (2.5,3) -- (0,3) -- (0,0);

\draw (4,0) -- (2.5,0) -- (2.5,3) -- (4,3) -- (4,0);

\end{tikzpicture}

\end{center}

By induction hypotheses we know that we can multiply a vector by $R \times X$ using $\alpha_{k-1}(|R|+|X|)$ gates. We can multiply a vector by $B \times X$ using $\alpha_{1} (|B| + |X|)$. To multiply a vector by $R \sqcup B \times X$ we can multiply it by submatrices separately and then combine it using additional $|X|$ gates. Together this gives us following relation:

\[ C((R \sqcup B \times X \sqcup Y)x ) \leq C((R \times X)x) + C((B \times X)x) + |X| + C((R \sqcup B \times Y)x) \leq \]

\[\leq \alpha_{k-1}(|X| + |R|) + \alpha_{1}(|X| + |B|) + |X| + C((R \sqcup B \times Y)x) \leq 3 \alpha_{k-1} |X| + \alpha_{k-1} n + C((R \sqcup B \times Y)x)\]
