...
\todo{V: My adaptation of the abstract starts here}
We study the complexity of \emph{dense matrices}, i.e. 0/1 matrices of size
$n \times n$ with $O(n)$ zeroes. More specifically, we are interested in
the complexity of computing the \emph{dense linear operator} $A\mathbf{x}$,
where $A$ is a dense matrix and~$\mathbf{x}$ is a vector over an arbitrary
semigroup: How many semigroup operations are required to simultaneously compute
all values of the resulting vector? We mainly concentrate on the case of idempotent semirings due to its importance.

Our two main results are: (i) we present a linear-size construction for the case
when the semigroup is commutative and not necessary idempotent, and (ii) we prove that the non-commutative case is strictly harder and is equivalent to the classic Range Queries problem~--~the corresponding $O(n\alpha(n))$-size circuits can be readily obtained by applying the Yao's Range Queries algorithm. In particular, we show that commutativity is a crucial property in this problem.
...
define as 0/1 matrices of size $n \times n$ that contain $O(n)$ zeroes. \todo{V: added}We are mainly interested in the computations in the algebraic structures lacking an inverse operation (semigroups). Thus, dense matrices being
opposite to \emph{sparse matrices} might intuitively seem harder in terms
...
be reduced to the sparse one by subtracting $y_i$ from the sum
...
presented linear-size construction. In particular, we \todo{V: I suggest to say ``we observe''}show that an $n\times n$
...
\todo[inline]{AK: Andrey, we should mention that counting triangles is not difficult even without our results\\V: We better say that counting is usually not difficult, cause we can use subtraction when counting}
