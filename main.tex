...
whose linear operator is computed trivially in $O(n)$ time, and $\overline{A}$ is the
...
with probability at least $1-\frac{\log^{O(1)}n}{n}$. There also
exists a~deterministic algorithm with running time $O(z+n\log^{O(1)}n)$.
\end{restatable}
...
and outputs a~circuit computing $Ax$ with probability at least $1-\frac{\log^{O(1)}n}{n}$. There also exists a~deterministic 
algorithm with running time $O(n\log^{O(1)}n)$.
\end{lemma}
...
To implement this algorithm in time $O(n\log^{O(1)}n)$, we store, for each column~$j$ of~$A$, a~sorted array of rows~$i$ such that $A_{ij}=0$. Since the total number of zeros~$z$ is at most~$n\log n$, these arrays can be computed in time $O(n\log^{O(1)}n)$: if $c_1, \dotsc, c_n$ are the numbers of zeros in the columns, then sorting the corresponding arrays takes time 
\[\sum_{i=1}^{n}r_i \log r_i \le \log(n \log n) \cdot \sum_{i=1}^{n}r_i \le \log(n \log n) \cdot n\log n \, .\]

At every iteration, we need to update the set~$R$. For this, we need to remove some rows from it (from the column that no longer belongs to the stripe of columns of width $\log t$) and to add the rows of the newly added column. Since the size of $|R|$ is always at most~$n$ and the total number of zeros is $z \le n\log n$, the total running time for all such updates is 
$O(n\log^2n)$ (if one uses, e.g., a~balanced binary search tree for
representing~$R$). 

If $|R| > \log t$, one just takes an all-one column (all such columns
can be stored in a~list). If $|R| \le \log t$, we need to find a~column outside of the set~$F$. For this, we just scan the list of the remaining columns. For each column, we first check whether it belongs to the set~$F$. This can be checked in time $O(\log^2t)$: for every row in~$|R|$, one checks whether this row belongs to the sorted array of the considered column using binary search in time
...
One deterministically generates a~circuit for~$A$ of size $O(n)$ in time $O(n\log^{O(1)}n)$ by Theorem~\ref{thm:upperbound}.
