...
whose complexity is $\Theta(n\alpha(n))$. However, for the case when the underlying 
...
using $O(n\alpha(n))$ semiring operations, where $\alpha(n)$
is the inverse Ackermann function. Moreover, there exists
a~matrix~$A \in \{0,1\}^{n \times n}$ with exactly two zeros
in every row such that the minimum number of semiring operations
...
Corollary~\ref{cor:matrixmultiplication} can be seen as a~natural 
special case where faster algorithms are possible.
