...
We prove a~sequence of more and more general results. In the first result, which is almost trivial,  commutativity is not even needed.
...
problem. We give a~sketch of its solution as below we prove a~more general results.
...
\todo[inline]{Andrey, rewrite this proof}
\end{proof}

We now proceed to the most general case.
\begin{theorem}
A~matrix $A \in \{0,1\}^{n \times n}$ with $k$~zeros can 
be computed by a~circuit of size $O(n+k)$.
\end{theorem}
\begin{proof}
Denote the set of rows and the set of columns of~$A$ by~$R$ and~$C$, respectively. Let $R_0 \subseteq R$ be all the rows having at least $\log n$ zeros and $R_1=R \setminus R_0$. We compute the matrices $R_0 \times C$ and $R_1 \times C$ separately.

\emph{Computing $R_0 \times C$.} Thanks to 
Observation~\ref{obs:transpose}, it suffices 
to compute $C \times R_0$. Let 
$|R_0|=t$. Clearly, $t \le k/\log n$. Since $C \times R_0$ 
contains at most~$k$ zeroes, it contains at most $O(n+k)$ 
ranges and it is enough to compute all of them. We compute all 
of them in the divide-and-conquer fashion. Namely, assume that 
the columns have indices $1, \dotsc, t$ and split them into two 
halves: $1, \dotsc, t/2$ and $t/2+1,\dotsc, t$. Using $O(t)$
gates one can compute all suffixes of the left half and all 
prefixes of the right half. Using these precomputed suffixes and 
prefixes one can answer any query $(l,r)$ such that $l \le t/2 
\le r$ in a~single additional gate. It remains to answer all the queries that lie entirely in one of the halves. We do this by constructing recursively circuits for both halves. The resulting recurrence relation $T(t) \le 2T(t/2)+O(t)$ implies that the resulting circuit has size at most
\[O(n+k+t\log t)=O\left(n+k+\frac{k}{\log n} \cdot \log k\right)=O(n+k)\, ,\]
since $k \le n^2$.

\emph{Computing $R_1 \times C$.}
\todo[inline]{TBW}
\end{proof}
...
We thank Paweł Gawrychowski for pointing us out to the paper~\cite{}.
