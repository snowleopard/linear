...
and others. It is immediate that $O(t_1+n+m)$ semigroup operations are
sufficient. The main question studied in this paper is:
can $Ax$~be computed using $O(t_0+n+m)$ semigroup operations?
...
More specifically, we are interested in lower and upper bounds
...
\todo{Sasha: why use $n^{O(1)}$ here instead of simpler $O(n)$ as above?}
