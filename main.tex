...
an $n\times n$ matrix can be multiplied by a dense matrix over an arbitrary commutative semiring in $O(n^2)$ time.

% Volodya, I commented this out, because we don't specifically work with Boolean
% or tropical semirings in the paper. I agree these are interesting semirings,
% but so are many others.
% We note that our positive results work in particular for computation of linear
% operators over Boolean and tropical semirings\todo{V: added}.
\end{abstract}
...
The interest to such computations has recently grew substantially throughout Computer Science community (see, for example,~\cite{Jukna16,Williams14,butkovic10systems})\todo{V: added}.
...
%As an additional contribution, we prove that the same bound $\Theta(n\alpha(n))$ also holds for the case of \emph{idempotent} non-commutative semigroups, which are significantly more powerful.
...
In this section we discuss some motivation and demonstrate two applications of the presented linear-size
...
further assume that a dense 0-1 matrix has $O(n)$ zero elements.

Note that multiplication of dense matrices by vectors can be viewed as a special case of Range Queries. Indeed, we can split the rows of a dense matrix into ranges, compute answers to all these range queries and then compute the rows of the matrix combining corresponding ranges.

As mentioned above, over algebraic structures with inverse operators the computations with dense matrices does not differ substantially from the computations with sparse matrices. However, the picture is completely different when the computation is performed over semirings, due to the lack of inverse operation, and the computational complexity of various problems can differ substantially from more classical settings. Understanding complexity of computational tasks in the lack of inverse operations is a recurring topic in the recent years (see, for example,~\cite{AkianGG12,Williams14,GrigorievP15}) and the problems we are considering is a step in this direction. As far as we know, the complexity of the problem under consideration was not known even for the simplest semirings like $(\mathbb{B},\vee)$.



\subsection{Dense and boring matrix multiplication}\label{sec-boring-matrices}

Out first main result presented in Section~\ref{sec-commutative} allows us to obtain a
...
takes $O(n)$ time. As a consequence, we can multiply a 0-1~dense matrix $A$ by
...
$V \subseteq U$ and $E \subseteq V \times V$. Also recall that the graph overlay
...
which allows us to ``connect'' two graphs by adding edges from every vertex in
the left-hand graph to every vertex in the right-hand graph, possibly
...
structure not a~semiring according to the standard definition.
% We refer the
% reader to~\cite{mokhov2017algebraic} for further details about the algebraic
% properties and applications of this algebra of graphs.
...
Thanks to our linear-size construction for the dense linear operator, the size
...
the alphabet $\{x_1,\ldots,x_n\}$ we have $W=W'$ in $X_S$ iff no matter which
...
(respectively, idempotent), then $X_S$ is also commutative (respectively,
idempotent).
...
of variables in the equivalence separates the parts of the equivalence that
cannot be affected by the rest of the variables and must therefore be the
equivalences themselves. We also note that this definition exactly captures the
idempotent case: for a free idempotent semigroup the conditions in this
definition are ``if and only if''\cite{GreenR52}.
...
idempotent group with $n$ generators (if $S$ is idempotent, then $X_S$ is also
...
letters are also equal by the Definition~\ref{def:faithful}.
