...
Let $(S, \circ)$~be a~commutative semigroup, $x \in S^n$ be a~vector of formal
variables, and $A \in \{0,1\}^{m \times n}$ be a~matrix with~$t_0=n^{O(1)}$
\todo{Sasha: why use $n^{O(1)}$ here instead of simpler $O(n)$ as above? Andrey, by writing $n^{O(1)}$ I mean polynomial, but not linear.}
