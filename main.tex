...
multiplication of dense and, more generally, \emph{boring matrices} over
arbitrary semirings (Section~\ref{sec-boring-matrices}) and compact algebraic
representation of dense graphs (Section~\ref{sec-dense-graph}).
...
matrix into ranges, compute answers to all these range queries, and then compute
the rows of the matrix combining the corresponding ranges.

As mentioned above, computations on dense matrices over algebraic structures
with inverse operations do not differ substantially from computations on sparse
matrices. However, the situation is very different when the computation is
performed over semigroups or semirings, due to the lack of inverse operations,
and the computation complexity of various problems can differ substantially from
more classical settings. Understanding complexity in the settings with the lack
of inverse operations is a recurring topic in the recent years (see, for
example,~\cite{AkianGG12,Williams14,GrigorievP15}) and the results we obtain in
this paper provide a further step in this research direction. As far as we know,
the complexity of the problem under consideration was not known even for the
simplest semigroups like $(\mathbb{B},\vee)$.
