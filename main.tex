...
\begin{abstract}
We study the complexity of \emph{dense} 0/1 matrices, i.e. $n \times n$ matrices
that contain $O(n)$ zeros. More specifically, we are interested in the
complexity of computing the dense linear operator $\mathbf{y} = A\mathbf{x}$,
...
all values of the resulting vector~$\mathbf{y}$?

Our two main results are: (i) we present a linear-size solution for the case
...
problem~--~the corresponding $O(n\alpha(n))$-size circuits are readily obtained
by applying the Yao's Range Queries algorithm.

As a simple application of the presented linear-size construction, we show how
to multiply dense $n\times n$ matrices over an arbitrary semiring in $O(n^2)$
time.
\end{abstract}
