...
\begin{abstract}
Let $A \in \{0,1\}^{n \times n}$ be a~matrix with $t_0$~zeroes
...
and others. It is easy to compute $Ax$ using $O(t_1)$ semigroup
operations.
The main question studied in this paper is:
can $Ax$~be computed using $O(t_0)$ semigroup operations?
...
(i.e. a~complement if a sparse matrix). Note that this covers the 
...
Let $A \in \{0,1\}^{n \times n}$ be a~matrix with $t_0$ zeroes
and $t_1$ ones and $x=(x_1, \dotsc, x_n) \in S^n$~be an~$n$-
dimensional vector
...
$O(t_1)$ semigroup operations.
The main question studied in this paper is:
can $Ax$~be computed using $O(t_0)$ semigroup
...
Our first result states that this 
is also possible for \emph{commutative} semigroups. Basically, it 
shows that in the commutative case complements of sparse 
...
to show that $Ax$ for a~matrix $A \in \{0,1\}^{m \times n}$ with $t_0=\Omega(n)$ zeros can be computed using $O(m+t_0)$ semigroup operations.
\item We assume that $t_0=\Omega(n)$ to be able to state an upper bound $O(t_0)$ instead of $O(t_0+n)$. Note that when
$t_0<n$, the matrix~$A$ is forced to contain all-one rows
that can be computed trivially.
\item By saying ``can be computed using $O(t_0)$ semigroup 
operation'' we mean that there exists a~\emph{circuit} of size 
$O(t_0)$ whose inputs are $x_1, \dotsc, x_n$ and where each 
...
a~matrix $A \in \{0,1\}^{n \times n}$ with $t_0$
zeros
and a~matrix $B \in S^{n \times k}$ and computes
the matrix $AB$ in time $O(t_0k)$.
\end{restatable}
