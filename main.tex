...
\begin{definition}
A commutative semigroup $(S, \circ)$ is faithful commutative if for any equivalence $W\equiv W'$ in $X_S$ we have $\var(W)=\var(W')$. 
\end{definition}
...
We say that a semigroup $X$ with generators $x_1,\ldots, x_n$ is strongly non-commutative if for any words $W$ and $W'$ in in the alphabet $\{x_1,\ldots, x_n\}$ the equivalence $W\equiv W'$ holds in $X$ only if the initial marks of $W$ and $W'$ are the same, terminal marks are the same, the equivalence $U \equiv U'$ holds in $X$, where $U$ and $U'$ are initials of $W$ and $W'$ respectively, and the equivalence $V \equiv V'$ holds in $X$, where $V$ and $V'$ are terminals of $W$ and $W'$ respectively.
\end{definition}

Basically, this definition states that the first and the last occurrence of variables in the equivalence separates the pieces of equivalence that cannot be affected by the rest of the variables and must be the equivalences themselves.
...
Suppose $X$ is strongly non-commutative. Suppose the equivalence $W \equiv W'$ holds in $X$ and $\var(W)=\var(W')=k$. Suppose $U$ and $U'$ are minimal (maximal) prefixes of $W$ and $W'$ such that $|\var(U)| = |\var(U')| = l\leq k$. Then the equivalence $U \equiv U'$ holds in $X$. The same is true for suffixes.
\end{lemma}
...
Suppose $X$ is strongly non-commutative. Suppose $W \equiv W'$ holds in $X_S$. Let us write down the variables of $W$ in the order in which they appear first time in $W$ when we read it from left to right. Let's do the same for $W'$. Then we obtain exactly the same sequences of letters.
...
For any faithful (non-commutative) semigroup $S$ there is a circuit to compute any dense operator of size $O(n\alpha(n))$. On the other hand, there exist dense matrices~$A$ such that any circuit computing $Ax$ must have size $\Omega(n\alpha(n))$.
\end{theorem}
...
Thus we will concentrate on lower bounds. We will view the computation of the circuit as a computation in a strongly non-commutative semigroup $X$. First we will prove the lower bound only for the case of idempotent strongly non-commutative semigroups $X$. In Section~\ref{sec:noncommutative_extension} we observe how to extend it to general strongly non-commutative $X$.
...
Commutative range queries problem has size $S$ circuits iff non-commutative dense linear operator problem has size $S$ circuits.
\end{theorem}
...
\section{General Non-commutative Case}\label{sec:noncommutative_extension}

Now we will show how to extend Theorem~\ref{thm:noncommlowerbound} beyond idempotent semirings.
